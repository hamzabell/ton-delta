...
const V1_PAIRS = [
  { id: "dogs-ton", label: "DOGS / TON", type: "meme" },
  { id: "not-ton", label: "NOT / TON", type: "meme" },
  { id: "redo-ton", label: "REDO / TON", type: "meme" },
];

export default function TransparencyPage() {
  const [selectedPairId, setSelectedPairId] = useState("dogs-ton");
  const [isSheetOpen, setIsSheetOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");

  const selectedPair =
    V1_PAIRS.find((p) => p.id === selectedPairId) || V1_PAIRS[0];
  ...
  const getLedger = (type: string) => [
    {
      leg: "Vault",
      provider: "TON W5",
      action: "Non-Custodial AA",
      status: "Secure",
    },
    {
      leg: "Watchman",
      provider: "n8n Self-Hosted",
      action: "Delta Rebalance Webhook",
      status: "Monitoring",
    },
    {
      leg: "Short",
      provider: "Storm Trade",
      action: "1x Basis Hedge",
      status: "Active",
    },
    {
      leg: "Settlement",
      provider: "Distributor",
      action: "Atomic 80/20 Profit Split",
      status: "Verified",
    },
  ];
    ...
    <div className="space-y-8 pb-24">
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4">
        <div className="bg-white/[0.03] border border-white/5 rounded-3xl sm:rounded-[2rem] p-4 sm:p-6 relative overflow-hidden group">
          ...
          <p className="text-[8px] sm:text-[9px] font-black text-white/20 uppercase tracking-widest mb-1 sm:mb-2 text-nowrap">
            Strategy Health
          </p>
          <h3 className="text-2xl sm:text-4xl font-black text-white italic tracking-tighter">
            100.0%
          </h3>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {getLedger(selectedPair.type).map((item, i) => (
            <div
              key={i}
              className="bg-white/[0.03] border border-white/5 rounded-2xl p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6 group hover:bg-white/[0.05] transition-all"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[9px] font-black text-white/20 uppercase tracking-[0.1em] mb-0.5">
                    {item.leg} • {item.provider}
                  </p>
              ...
              <div className="flex sm:justify-end border-t sm:border-t-0 border-white/5 pt-3 sm:pt-0">
                <span className="text-[10px] font-black text-[#E2FF00] uppercase tracking-widest italic">
                  {item.status}
                </span>
              </div>
            </div>
            ...
            <div className="px-8 pt-8 pb-10 space-y-8 flex flex-col overflow-hidden">
              <div className="flex items-center justify-between shrink-0">
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-[#E2FF00] font-black uppercase tracking-widest">
                    Select Pair To Verify
                  </p>
                </div>
                <button
                  onClick={() => setIsSheetOpen(false)}
